...
export const getPicoResult = async (input: ImageInfo, cb: (progress: number) => void): Promise<VoteResult> => waitForConnection()
    .then(() => secretariumHandler.request(klaveContract, 'getPicoResult', {
        ...input
    }, `getPicoResult-${Math.random()}`))
    .then(tx =>
        new Promise((resolve, reject) => {
            tx.onResult((result: any) => {
